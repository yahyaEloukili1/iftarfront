...
isLastRowOfGroup(index: number) {
  const currentCin = this.benificiaires[index].cin;
  const nextCin = this.benificiaires[index + 1]?.cin;
...
getReources(){
  this.rnpService.getAllBenificiairesGroupedByCin('benif').subscribe(data=>{
